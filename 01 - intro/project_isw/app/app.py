...
def main():
    print_log("Starting the Quickteller Assistant application")
    st.title("Quickteller Assistant")
    ...
    subject = st.selectbox(
        "Select a subject:",
        ["isw_general_faq", "qt_rebirth_faq"]
    )
    print_log(f"User selected course: {subject}")
    ...
    if st.button("Ask"):
        ...
        with st.spinner('Processing...'):
            ...
            output = get_answer(user_input, subject)
            end_time = time.time()
            ...
            save_conversation(st.session_state.conversation_id, user_input, output, subject)
